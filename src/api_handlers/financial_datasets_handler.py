...
class FinancialDatasetsApiHandler(BaseApiHandler):
    """
    API handler for Financial Datasets API (financialdatasets.ai).
    """

    def __init__(self):
        """Initialize the handler with cache."""
        self._cache = get_cache()
        self._api_key = os.environ.get("FINANCIAL_DATASETS_API_KEY")
    ...
    def get_prices(self, ticker: str, start_date: str, end_date: str) -> List[Price]:
        ...
        url = f"{self._base_url}/prices/?ticker={ticker}&interval=day&interval_multiplier=1&start_date={start_date}&end_date={end_date}"
    ...
    def get_financial_metrics(
        ...
        url = f"{self._base_url}/financial-metrics/?ticker={ticker}&report_period_lte={end_date}&limit={limit}&period={period}"
        response = requests.get(url, headers=self._get_headers())
        if response.status_code != 200:
            raise Exception(f"Error fetching data: {ticker} - {response.status_code} - {response.text}")
    ...
    def search_line_items(
        ...
        url = f"{self._base_url}/financials/search/line-items"
    ...
    def get_insider_trades(
        ...
        while True:
            url = f"{self._base_url}/insider-trades/?ticker={ticker}&filing_date_lte={current_end_date}"
            if start_date:
        ...
        # Cache the results
        self._cache.set_insider_trades(ticker, [trade.model_dump() for trade in all_trades])
        return all_trades
    ...
    def get_company_news(
        ...
        while True:
            url = f"{self._base_url}/news/?ticker={ticker}&end_date={current_end_date}"
            if start_date:
        ...
        # Cache the results
        self._cache.set_company_news(ticker, [news.model_dump() for news in all_news])
        return all_news
